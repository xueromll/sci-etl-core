...
class TestAsyncArxivParseListing:
    def test_parses_all_new_entries(self, mocker):
        ...
        records, total = extractor._parse_listing(ATOM.encode())
        assert total == 2
    ...
    @pytest.mark.asyncio
    async def test_fetch_page_requests_the_cursor_offset_and_points_past_the_page(self, mocker):
        client = _client(mocker, resp=_resp(mocker, 200, ATOM.encode()))
    ...
    @pytest.mark.asyncio
    async def test_fetch_page_starts_at_offset_zero_and_an_empty_page_ends_the_listing(self, mocker):
        client = _client(mocker, resp=_resp(mocker, 200, b"<feed xmlns='http://www.w3.org/2005/Atom'></feed>"))
        extractor = _build(client, mocker)
    ...
    @pytest.mark.asyncio
    async def test_fetch_page_rejects_a_cursor_that_is_not_an_offset(self, mocker):
    ...
    def test_a_negative_offset_has_no_cursor(self, mocker):
        with pytest.raises(ValueError, match="must not be negative"):
            _build(_client(mocker), mocker).cursor_for_offset(-1)
    ...
    def test_skips_duplicate_base_ids_within_same_feed(self, mocker):
            ...
            extractor = _build(_client(mocker), mocker)
            records, total = extractor._parse_listing(feed.encode())
            assert total == 2
    ...
    def test_empty_feed_returns_no_records(self, mocker):
        extractor = _build(_client(mocker), mocker)
        records, total = extractor._parse_listing(b"<feed xmlns='http://www.w3.org/2005/Atom'></feed>")
        assert records == []
        assert total == 0
    ...
    def test_malformed_payload_raises_instead_of_reporting_end(self, mocker, payload):
        ...
        with pytest.raises(MalformedResponseError):
            extractor._parse_listing(payload)
...
class TestAsyncArxivListingMetadata:
    def test_holds_categories_authors_published_and_year(self, mocker):
        ...
        (record,), _ = _build(_client(mocker), mocker)._parse_listing(feed)
    ...
    def test_an_entry_without_categories_or_authors_gets_empty_lists(self, mocker):
        (record,), _ = _build(_client(mocker), mocker)._parse_listing(_single_entry_feed(""))
    ...
    def test_a_missing_or_blank_published_date_omits_published_and_year(self, mocker, published):
        (record,), _ = _build(_client(mocker), mocker)._parse_listing(_single_entry_feed(published))
