...
class TestPipelineHappyPath:
    ...
    @pytest.mark.asyncio
    async def test_skips_irrelevant_without_fetching_or_exporting(self, mocker):
    ...
    @pytest.mark.asyncio
    async def test_no_export_when_extraction_yields_nothing(self, mocker):
    ...
    @pytest.mark.asyncio
    async def test_stops_when_listing_is_exhausted(self, mocker):
...
class TestPipelineResilience:
    @pytest.mark.asyncio
    async def test_one_record_failure_does_not_abort_run(self, mocker):
...
class TestPipelineConcurrency:
    @pytest.mark.asyncio
    async def test_shared_processed_ids_has_no_lost_updates(self, mocker):
        record_count = 200
        pipeline, _, _, _, _, state = _build(mocker, _records(record_count), max_workers=16)
        assert await pipeline.run(query="q", max_records=record_count, sleep_between=0) == record_count
    ...
    @pytest.mark.asyncio
    async def test_respects_max_records_ceiling_under_concurrency(self, mocker):
